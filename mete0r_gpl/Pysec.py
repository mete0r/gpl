...
# See http://www.valuedlessons.com/2008/02/pysec-monadic-combinatoric-parsing-in.html # noqa
...
class StateChanger(Record("changer", "bindees"), Monad):
    ...
    def run(self, state0):
        value, state = (self.changer(state0) if callable(self.changer)
                        else self.changer)
...
def match(state0, expected):
    ...
    if actual == expected:
        ...
        raise ParseFailed("expected %r, actual %r"
...
def pair(parser1, parser2):
    ...
    return parser1 >> (lambda value1: parser2
                       >> (lambda value2: Parser.unit((value1, value2))))
...
def sep_end_by(parser, sep_parser, min_count=0):
    return skip_after(sep_by(parser, sep_parser, min_count),
                      option(None, sep_parser))
...
def satisfy(name, passes):
    return tokens(1, lambda char: (True, char)
                  if passes(char) else (False, "not " + name))
...
def none_of(chars):
    char_set = frozenset(chars)
    return satisfy("not one of %r" % chars,
                   lambda char: char and char not in char_set)
...
# , digits, between, pair, spaces, match, quoted_collection, sep_end_by

# HACK: json_choices is used to get around mutual recursion
# a json is value is one of text, number, mapping, and collection, which
# we define later
json_choices = []
...
# in case you're unfamiliar with monads, "parser >> Parser.lift(func)" means
# "pass the parsed value into func but give me a new Parser back"
number = group_chars([option_chars(["-"]), digits,
                      option_chars([".", digits])]) >> Parser.lift(float)

# quoted_collection(start, space, inner, joiner, end) means "a list of inner
# separated by joiner surrounded by start and end"
# also, we have to put a lot of spaces in there since JSON allows lot of
# optional whitespace
...
if __name__ == '__main__':
    print json.parseString("{'a' : -1.0, 'b' : 2.0, "
                           "'z' : {'c' : [1.0, [2.0, [3.0]]]}}")
